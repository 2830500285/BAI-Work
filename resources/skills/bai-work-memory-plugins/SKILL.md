...
If those values are missing, continue with BAI Work built-in memory and explain that YantrikDB is not configured.
...
记忆功能默认使用 BAI Work 内置能力。YantrikDB 和 Icarus 是可选增强；只有在实际配置存在时才使用，不要假装可用。

...
function Load-LocalReleaseEnv([string]$RootPath) {
  $configured = [Environment]::GetEnvironmentVariable('BAI_WORK_RELEASE_ENV', 'Process')
  $candidates = @()
...
$RequestedChannel = if ($Stable) {
  ...
  $env:RELEASE_CHANNEL
} elseif ($env:BAI_WORK_UPDATE_CHANNEL) {
  $env:BAI_WORK_UPDATE_CHANNEL
} else {
  'frontier'
}
...
$env:BAI_WORK_APP_VERSION = $ReleaseVersion
$env:RELEASE_CHANNEL = $ReleaseChannel
$env:BAI_WORK_UPDATE_CHANNEL = $ReleaseChannel
Write-Info "App version: $env:BAI_WORK_APP_VERSION"

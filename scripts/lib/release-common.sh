...
release_export_update_channel() {
  ...
  export BAI_WORK_UPDATE_CHANNEL="${RELEASE_CHANNEL}"
...
release_load_local_env() {
  local env_file="${BAI_WORK_RELEASE_ENV:-}"
...
release_compute_version() {
  ...
  if [[ "${RELEASE_BUMP:-}" == "none" && -n "${RELEASE_TAG:-}" ]]; then
    ...
    RELEASE_NAME="BAI Work ${RELEASE_VERSION}"
    LATEST_TAG=""
    return
  fi
...
release_export_app_version() {
  ...
  export BAI_WORK_APP_VERSION="${RELEASE_VERSION}"
  cyan "  App:     ${BAI_WORK_APP_VERSION}"
}
...
release_clean_dist_artifacts() {
  ...
  rm -f "${ROOT}"/dist/BAI-Work-* "${ROOT}"/dist/BAI\ GUI-* "${ROOT}"/dist/latest*.yml "${ROOT}"/dist/*.blockmap
}
...
release_write_notes_file() {
...

  {
    ...
    echo "- 平台: ${RELEASE_PLATFORMS_NOTE:-Mac Intel x64}"
  } >>"${dest}"
}

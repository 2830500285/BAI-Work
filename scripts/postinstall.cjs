...
// BAI Work may spawn local runtime helpers with the Electron binary
...
try {
  ...
  if (result.status !== 0) {
    console.warn('[postinstall] better-sqlite3 electron prebuild failed; BAI Work will use the JSONL fallback')
  }
} catch (error) {

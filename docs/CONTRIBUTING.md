...
Thank you for contributing to BAI Work.
...
For BAI Work, taste means clear workflows, restrained interfaces, humane copy, and behavior that feels obvious after one use. Strong contributions show judgment, not just implementation.
...
- feature branches: optional short-lived branches created from `main`

Rules:

- Prefer starting work from the latest `main`
- If you create a feature branch, branch off from `main`
- Open pull requests into `main` unless maintainers explicitly request another base branch
...
A well-structured PR for BAI Work is focused and self-contained. It typically:
...
For most contributions, opening the PR from a short-lived feature branch is preferred over pushing directly to `main`.
...
- `fix: handle missing BAI Code runtime path`
...
- BAI Work version (from Settings or the About dialog)
- Bundled BAI Code version (`baicode --version` in the runtime directory, if available)
...
contribution as part of BAI Work under commercial, proprietary, noncommercial, or
other license terms.

...
const RECOMMENDED_ITEMS: MarketplaceItem[] = [
  ...
  },
  {
    id: 'serena',
    kind: 'mcp',
    titleKey: 'pluginMcpSerenaTitle',
    descriptionKey: 'pluginMcpSerenaDesc',
    group: 'recommended',
    sourceLabelKey: 'pluginSourceSerenaOfficial',
    detail: 'https://oraios.github.io/serena/02-usage/030_clients.html',
    mcpConfig: (workspaceRoot) =>
      buildMcpConfig(
        'serena',
        'serena',
        ['start-mcp-server', '--context=ide', '--project', workspaceRoot || '/path/to/workspace'],
        {
          trustScope: 'workspace',
          trustedWorkspaceRoots: workspaceRoot ? [workspaceRoot] : undefined
        }
      )
  },
  {
  ...
  },
  {
    id: 'firecrawl',
    kind: 'mcp',
    titleKey: 'pluginMcpFirecrawlTitle',
    descriptionKey: 'pluginMcpFirecrawlDesc',
    group: 'recommended',
    sourceLabelKey: 'pluginSourceFirecrawlOfficial',
    detail: 'https://github.com/firecrawl/firecrawl-mcp-server',
    mcpConfig: () =>
      buildMcpConfig(
        'firecrawl',
        'npx',
  ...
  },
  {
    id: 'litellm-gateway',
    kind: 'skill',
    titleKey: 'pluginSkillLiteLlmTitle',
    descriptionKey: 'pluginSkillLiteLlmDesc',
    group: 'recommended',
    sourceLabelKey: 'pluginSourceLiteLlmOfficial',
    detail: 'https://docs.litellm.ai/docs/',
    skillInstructions: [
      'Use LiteLLM only when the user wants provider routing, fallback, budgets, or an OpenAI-compatible local gateway.',
      '',
      'Before changing provider settings, confirm that the user has started a LiteLLM proxy and has chosen the models to expose. A typical local endpoint is http://127.0.0.1:4000/v1.',
      '',
      'Never write API keys to the repository, chat transcript, or generated files. Ask the user to configure provider credentials through BAI Work settings or environment variables.',
      '',
      'When diagnosing model failures, separate BAI Work provider configuration issues from LiteLLM proxy issues. Check base URL, model alias, and proxy health first.'
    ].join('\n')
  },
  {
    id: 'langfuse-observability',
    kind: 'skill',
    titleKey: 'pluginSkillLangfuseTitle',
    descriptionKey: 'pluginSkillLangfuseDesc',
    group: 'recommended',
    sourceLabelKey: 'pluginSourceLangfuseOfficial',
    detail: 'https://langfuse.com/docs/observability/get-started',
    skillInstructions: [
      'Use Langfuse only when the user explicitly wants tracing, prompt/version observability, evaluation datasets, or LLM debugging telemetry.',
      '',
      'Do not enable telemetry by default. Require the user to provide a Langfuse host and credentials through local settings or environment variables.',
      '',
      'Treat prompts, completions, tool outputs, file paths, and API keys as sensitive. Redact secrets and explain what data will be sent before enabling any export.',
      '',
      'For local-only debugging, prefer BAI Work logs first. Add Langfuse only when the user needs persistent trace history or team observability.'
    ].join('\n')
  },
  {
    id: 'local-security-scan',
    kind: 'skill',
    titleKey: 'pluginSkillSecurityScanTitle',
    descriptionKey: 'pluginSkillSecurityScanDesc',
    group: 'recommended',
    sourceLabelKey: 'pluginSourceSecurityTools',
    detail: 'https://github.com/semgrep/semgrep',
    skillInstructions: [
      'Use this skill for local security review of the current workspace. Prefer local tools and do not upload source code, dependency manifests, or findings unless the user explicitly asks.',
      '',
      'Suggested scanners when installed: semgrep scan --config auto, gitleaks detect, osv-scanner scan source, and trivy fs. Run only the scanners that are available locally or that the user asks to install.',
      '',
      'Never print secret values. If a scanner reports a credential, redact the value and report the file path, line, scanner name, and remediation.',
      '',
      'Prioritize actionable findings: exposed credentials, unsafe command execution, dependency vulnerabilities, dangerous deserialization, injection, and insecure update or plugin loading paths.',
      '',
      'If a scanner is missing, report the missing tool and the exact install command separately instead of failing the whole review.'
    ].join('\n')
  },
  {
    id: 'crawl4ai-web-ingestion',
    kind: 'skill',
    titleKey: 'pluginSkillCrawl4aiTitle',
    descriptionKey: 'pluginSkillCrawl4aiDesc',
    group: 'recommended',
    sourceLabelKey: 'pluginSourceCrawl4aiOfficial',
    detail: 'https://docs.crawl4ai.com/',
    skillInstructions: [
      'Use Crawl4AI for structured web extraction, markdown conversion, and RAG-ready ingestion when a local or self-hosted Crawl4AI service is available.',
      '',
      'Keep crawl scope explicit: list the starting URLs, allowed domains, depth, and file types before running a crawl.',
      '',
      'Do not scrape authenticated, private, or paywalled pages unless the user confirms they have rights to access and process that content.',
      '',
      'Prefer concise extracted markdown, source URLs, and a short data-quality note over dumping raw crawler output into the chat.',
      '',
      'If the user wants MCP wiring, connect only to the user-provided Crawl4AI MCP endpoint. Do not assume a default remote endpoint.'
    ].join('\n')
  },

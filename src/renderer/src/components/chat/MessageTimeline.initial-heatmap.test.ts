...
describe('MessageTimeline initial heatmap empty hero routing', () => {
  ...
  it('shows the BAI Work wordmark hero for eligible initial chat states', () => {
  ...
  it('keeps offline, missing-workspace, and Claw empty states gated away from the heatmap', () => {
    ...
    expect(offlineHtml).toContain('BAI Work is waking the local agent')
    expect(offlineHtml).toContain('aria-label="BAI Work"')

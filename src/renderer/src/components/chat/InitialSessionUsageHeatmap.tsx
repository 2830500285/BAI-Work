...
function usageTotalsFromBuckets(buckets: DailyUsageBucket[]): UsageTotalsBucket {
  ...
  let hasTokenEconomyTelemetry = false
  const totals = buckets.reduce<UsageTotalsBucket>(
    (acc, bucket) => {
      ...
      if (bucket.cacheTelemetryAvailable) hasCacheTelemetry = true
      if (bucket.tokenEconomyTelemetryAvailable) hasTokenEconomyTelemetry = true
  ...
  return {
    ...
    cacheTelemetryAvailable: hasCacheTelemetry,
    tokenEconomyTelemetryAvailable: hasTokenEconomyTelemetry,
    cacheHitRate: hasCacheTelemetry && cacheTotal > 0 ? totals.cachedTokens / cacheTotal : null
  }
}
...
function dailySummary(
  ...
  const key = bucket.cacheTelemetryAvailable ? 'usageHeatmapDaySummary' : 'usageHeatmapDaySummaryNoCache'
  return t(key, {
    date: bucket.date,
    tokens: formatCompactNumber(bucket.totalTokens),
    cost: formatCost(bucket.costUsd, locale, bucket.costCny),
    saved: bucket.cacheTelemetryAvailable ? formatCompactNumber(bucket.cachedTokens) : '-',
    turns: bucket.turns,
...
function modelUsageBreakdownSummary(
  ...
  bucket: Pick<
    DailyUsageBucket,
    'inputTokens' | 'outputTokens' | 'cachedTokens' | 'cacheMissTokens' | 'totalTokens' | 'cacheTelemetryAvailable'
  >,
  t: (key: string, values?: Record<string, unknown>) => string,
  locale: string
): string {
  if (!bucket.cacheTelemetryAvailable) {
    return t('usageHeatmapModelTooltipNoCache', {
      label,
...
function modelUsageChartBreakdown(
  ...
  const cachedInput = bucket.cacheTelemetryAvailable ? Math.max(0, bucket.cachedTokens) : 0
  const uncachedInput = Math.max(
    0,
    bucket.cacheTelemetryAvailable && bucket.cacheMissTokens > 0 ? bucket.cacheMissTokens : bucket.inputTokens - cachedInput
  )
...
function ModelUsagePanel({
  ...
  const tooltipRows = activeBreakdown
    ? activeDay?.cacheTelemetryAvailable
      ? [
          {
            key: 'cached-input',
      ...
      : [
          {
            key: 'input',
            label: t('usageHeatmapModelTooltipInput'),
            value: activeBreakdown.uncachedInput,
            color: MODEL_USAGE_BREAKDOWN_COLORS.uncachedInput
          },
          {
      ...
      <div className="grid min-h-[206px] grid-cols-[44px_1fr] gap-2">
        ...
        <div className="relative min-w-0" onMouseLeave={() => setActiveDayIndex(null)}>
          ...
          {chartDays.map((bucket, index) => {
            ...
            const segments = bucket.cacheTelemetryAvailable ? [
              {
                key: 'output',
          ...
          return (
            ...
            >
              ...
              <span className="min-w-0 truncate whitespace-nowrap text-right tabular-nums text-ds-faint">
                {bucket.cacheTelemetryAvailable
                  ...
                  : t('usageHeatmapModelTokenBreakdownNoCache', {
                      input: formatCompactNumber(bucket.inputTokens),
                      output: formatCompactNumber(bucket.outputTokens)
                    })}
              </span>
...
export function InitialSessionUsageHeatmapView({
  ...
  const overviewMetrics = [
    ...
    { label: t('usageHeatmapCost'), value: formatCost(totals.costUsd, i18n.language, totals.costCny) },
    {
      ...
      value: totals.cacheTelemetryAvailable
        ? t('usageHeatmapSavedTokensValue', { tokens: formatCompactNumber(totals.cachedTokens) })
        : '-'
    },
    {
      label: t('usageHeatmapContextSavings'),
      value: totals.tokenEconomyTelemetryAvailable
        ? t('usageHeatmapSavedTokensValue', {

...
describe('MessageTimelineEmptyHero — runtime offline hero (issue #78)', () => {
  ...
  it('uses the waking title when no runtime error is available', () => {
    ...
    expect(html).toContain('BAI Work is waking the local agent')
  ...
  it('switches to the error title and surfaces the localized error when a runtime error is provided', () => {
    ...
    expect(html).not.toContain('BAI Work is waking the local agent')
  ...
  it('uses the BAI Work wordmark without the legacy window mascot art', () => {
    // Still probing: stage carries the is-waking modifier, but the visible
    // artwork is the BAI Work brand card rather than the old window/Kun scene.
    ...
    expect(errored).toContain('aria-label="BAI Work"')

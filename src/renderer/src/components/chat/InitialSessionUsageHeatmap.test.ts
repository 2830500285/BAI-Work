...
function bucket(date: string, totalTokens: number, turns = 1) {
  return {
    ...
    cacheTelemetryAvailable: totalTokens > 0,
    tokenEconomyTelemetryAvailable: true,
    turns,
...
describe('InitialSessionUsageHeatmap', () => {
  ...
  it('renders the static BAI Work wordmark without usage controls in the default hero', () => {
  ...
  it('renders the usage panel without the animated hero in focus mode', () => {
    ...
    expect(html).toContain('Daily BAI usage calendar')
    ...
    expect(html).not.toContain('Collapse calendar')
    expect(html).not.toContain('Expand calendar')
    expect(html).not.toContain('aria-label="BAI Work"')
  })

  it('does not show zero cache savings when BAI Work usage is estimated without cache telemetry', () => {
    const estimatedBucket = {
      ...bucket('2026-05-01', 3100, 3),
      cachedTokens: 0,
      cacheMissTokens: 0,
      tokenEconomySavingsTokens: 0,
      cacheHitRate: null,
      cacheTelemetryAvailable: false,
      tokenEconomyTelemetryAvailable: false
    }
    const estimatedUsage = usage([estimatedBucket])
    estimatedUsage.totals = {
      ...estimatedUsage.totals,
      cachedTokens: 0,
      cacheMissTokens: 0,
      tokenEconomySavingsTokens: 0,
    ...
    const html = render(state({ usage: estimatedUsage, loaded: true }), { hideHero: true })

    expect(html).toContain('title="Cache hits saved">Cache hits saved</span><span class="mt-0.5 min-w-0 truncate text-[15px] font-semibold leading-5 tabular-nums text-ds-ink" title="-">-</span>')
    expect(html).toContain('title="Context saved">Context saved</span><span class="mt-0.5 min-w-0 truncate text-[15px] font-semibold leading-5 tabular-nums text-ds-ink" title="-">-</span>')
    expect(html).toContain('cache telemetry unavailable')
    expect(html).not.toContain('0 cached tokens')
    expect(html).not.toContain('0 tokens</span></span><span class="grid')
  })
  ...
  it('does not label model usage as zero cache hits when cache telemetry is unavailable', () => {
    const estimatedDay = {
      date: '2026-06-05',
      inputTokens: 2400,
      outputTokens: 700,
    ...
    const html = render(state({ usage: usage([estimatedDay]), loaded: true }), {
      hideHero: true,
      initialActiveTab: 'models',
      initialModelHoverIndex: 0,
      modelState: modelState({
    ...
    expect(html).not.toContain('0 hit')
    expect(html).not.toContain('0 miss')
    expect(html).not.toContain('Input (cache hit)')
    expect(html).not.toContain('Input (cache miss)')
  })
  ...
  it('renders loading, empty, and error states as calendar-only warmup states', () => {
    ...
    expect(loadingHtml).not.toContain('Daily BAI usage calendar')
  ...
  it('renders the BAI Work wordmark without a collapsed calendar card', () => {
    const html = render(state({ usage: usage(), loaded: true }))

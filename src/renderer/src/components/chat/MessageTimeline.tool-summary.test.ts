...
describe('MessageTimeline tool summaries', () => {
  ...
  it('labels the BAI Work runtime tool without exposing the baicode command name', () => {
    expect(
      summarizeToolBlock(
        toolBlock({
          summary: '正在写入 generated.py',
          toolKind: 'command_execution',
          meta: { toolName: 'bai_work' }
        }),
        t
      )
    ).toBe('BAI Work 正在写入 generated.py')
  })
})
...
describe('MessageTimeline Kun runtime metadata smoke', () => {
  ...
  it('hides BAI Work runtime instructions from user bubbles', () => {
    const block: ChatBlock = {
      kind: 'user',
      id: 'user_runtime',
      text: [

...
const labels: Record<string, string> = {
  ...
  kunBaseUrl: 'BAI base URL',
  kunBaseUrlDesc: 'BAI base URL description',
  ...
  runtimeEngineKun: 'Legacy runtime',
  runtimeEngineMimoWork: 'BAI Work runtime',
  mimoCredentialMode: 'BAI Code access',
  mimoCredentialModeDesc: 'BAI Code access description',
  ...
  mimoApiKey: 'BAI API key',
  mimoApiKeyDesc: 'BAI API key description',
  mimoApiKeyTokenplanWarning: 'Tokenplan keys usually start with tp-',
  mimoBaseUrl: 'BAI base URL',
  mimoBaseUrlDesc: 'BAI base URL description',
  mimoModel: 'BAI model',
  mimoModelDesc: 'BAI model description',
  port: 'Port',
  portDesc: 'Port description',
  kunBinary: 'BAI Code binary',
  kunBinaryDesc: 'BAI Code binary description',
  kunBinaryPlaceholder: 'Bundled BAI Code',
  kunDataDir: 'Data dir',
  ...
  kunDiagnostics: 'BAI diagnostics',
...
function baseCtx(): Record<string, unknown> {
  ...
  return {
    ...
    openPlugins: noop,
    mcpConfigPath: '/tmp/project/.bai-work/mcp.json',
...
describe('AgentsSettingsSection BAI diagnostics smoke', () => {
  ...
  it('builds a single patch when selecting the BAI preset model provider', () => {
    const provider = defaultModelProviderSettings()
    const bai = getModelProviderPreset('bai')
    expect(bai).not.toBeNull()
    const baiProvider = modelProviderPresetProfile(bai!, 'sk-bai')

    const patch = modelProvidersSettingsPatch({
      provider,
      providers: [baiProvider],
      kun: {
        providerId: baiProvider.id,
        model: baiProvider.models[0]
      }
    })

    expect(patch.provider?.providers).toEqual([expect.objectContaining({
      id: 'bai',
      baseUrl: 'https://api.b.ai/v1',
      endpointFormat: 'chat_completions',
      models: expect.arrayContaining(['claude-sonnet-4-6'])
    })])
  ...
  it('does not invent media generation settings when selecting the BAI text preset', () => {
    const provider = defaultModelProviderSettings()
    const bai = getModelProviderPreset('bai')
    expect(bai).not.toBeNull()
    const baiProvider = modelProviderPresetProfile(bai!, 'sk-bai')
    ...
    expect(patch.agents?.kun?.textToSpeech).toBeUndefined()
    expect(patch.agents?.kun?.musicGeneration).toBeUndefined()
    expect(patch.agents?.kun?.videoGeneration).toBeUndefined()
  })
  ...
  it('locks the default BAI provider id and hides the delete action', () => {
    const provider = defaultModelProviderSettings()
    const bai = getModelProviderPreset('bai')
    expect(bai).not.toBeNull()
    const html = renderToStaticMarkup(createElement(ProvidersSettingsSection, {
      ctx: {
        ...
        provider: {
          ...provider,
          providers: [modelProviderPresetProfile(bai!)]
        },
        kun: {
          ...defaultKunRuntimeSettings(),
          providerId: 'bai'
    ...
    const providerIdInput = html.match(/<input[^>]+value="bai"[^>]*>/)?.[0]
  ...
  it('renders BAI Work runtime credential controls when selected', () => {
    ...
    expect(html).toContain('BAI Work runtime')
    expect(html).toContain('BAI Code access')
    expect(html).toContain('BAI API key')
    expect(html).toContain('https://api.b.ai/v1')
  ...
  it('shows current BAI model compaction thresholds from the model profile', () => {
    const html = renderToStaticMarkup(createElement(AgentsSettingsSection, { ctx: baseCtx() }))
  ...
  it('renders MCP, Skill, web, attachment, and memory diagnostics', () => {
    ...
    expect(html).toContain('BAI diagnostics')
  ...
  it('describes MCP config as an external-tool JSON file instead of model credentials', () => {
    ...
    expect(html).toContain('/tmp/project/.bai-work/mcp.json')
    expect(html).toContain('Model and API credentials do not live in this MCP file')
    expect(html).not.toContain('BAI auth')
    expect(html).not.toContain('Base URL are stored in this file')
    expect(html).not.toContain('config.toml')
  })

  it('defines the BAI provider preset for the Providers menu', () => {
    const bai = getModelProviderPreset('bai')
    expect(bai && modelProviderPresetProfile(bai)).toMatchObject({
      id: 'bai',
      name: 'BAI',
      baseUrl: 'https://api.b.ai/v1',
      endpointFormat: 'chat_completions'

...
const TURN_MODEL_STORAGE_KEY = 'baiWork.turnModelLabel'
const CODE_WORKSPACE_ROOTS_STORAGE_KEY = 'baiWork.codeWorkspaceRoots.v1'
const LEGACY_CODE_WORKSPACE_ROOTS_STORAGE_KEY = 'kun.codeWorkspaceRoots.v1'
...
describe('chat-store Claw helpers', () => {
  ...
  it('compacts code workspace roots while excluding write, temporary, and Claw roots', () => {
    expect(
      compactCodeWorkspaceRoots([
        ...
        '/Users/zxy/project-a/',
        '/tmp/transient',
        '/Users/zxy/.bai-work/claw/agent/conversations/chat',
        '/Users/zxy/.bai-work/default_workspace',
        '~/.bai-work/write_workspace',
        '',
        '/Users/zxy/project-b'
      ])
    ).toEqual([
  ...
  it('filters internal BAI and legacy MIMO workspace aliases', () => {
    expect(
      compactCodeWorkspaceRoots([
        '~/.bai-work/default_workspace',
        'C:\\Users\\zxy\\.bai-work\\default_workspace',
        '~/.bai-work/default_workspace',
        'C:\\Users\\zxy\\.bai-work\\default_workspace\\',
        '/Users/zxy/project'
      ])
    ).toEqual(['/Users/zxy/project'])
  })

  it('ignores and removes legacy MIMO workspace-root cache entries', () => {
    localStorage.setItem(LEGACY_CODE_WORKSPACE_ROOTS_STORAGE_KEY, JSON.stringify([
      '/Users/zxy/BAI-Work-Shell'
    ]))
    localStorage.setItem(CODE_WORKSPACE_ROOTS_STORAGE_KEY, JSON.stringify([
      '/Users/zxy/bai-project'
    ]))

    expect(readCodeWorkspaceRoots()).toEqual(['/Users/zxy/bai-project'])
    expect(localStorage.getItem(LEGACY_CODE_WORKSPACE_ROOTS_STORAGE_KEY)).toBeNull()
  })
  ...
  it('recognizes Claw managed prompt summaries as Claw sessions', () => {
    expect(
      clawThreadTitleLooksManaged(`${CLAW_MANAGED_INSTRUCTIONS_HEADING} BAI Work scheduled-task tools`)
    ).toBe(true)
  ...
  it('keeps auto out of the composer pick list', () => {
    ...
    expect(pick).toContain('claude-sonnet-4-6')
    expect(pick).toContain('gpt-5.2')
  ...
  it('falls back to the runtime default model, then known defaults', () => {
    const pick = ['a-model', 'custom-model', 'claude-sonnet-4-6', 'gpt-5.2']

    expect(fallbackComposerModel(pick, 'custom-model')).toBe('custom-model')
    expect(fallbackComposerModel(pick, 'auto')).toBe('claude-sonnet-4-6')
    expect(fallbackComposerModel(pick, 'missing-model')).toBe('claude-sonnet-4-6')

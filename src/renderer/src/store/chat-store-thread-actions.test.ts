...
describe('chat-store-thread-actions queued messages', () => {
  ...
  it('does not queue GUI plan messages while another turn is active', async () => {
    ...
    const guiPlan: GuiPlanMessageContext = {
      operation: 'draft',
      workspaceRoot: '/workspace/bai-work',
      relativePath: '.bai-work/plan/feature.md',
      planId: 'plan-1',
    ...
    state.queuedMessages = [
      {
        ...
        guiPlan: {
          operation: 'draft',
          workspaceRoot: '/workspace/bai-work',
          relativePath: '.bai-work/plan/one.md',
          planId: 'plan-1'
        }
      },
      {
  ...
  it('applies the selected composer provider before sending a turn', async () => {
    ...
    const saveSettingsSilent = vi.fn(async () => ({
      agents: { kun: { providerId: 'bai', model: 'gpt-5.2' } },
      codePromptPrefix: ''
    }))
    ...
    vi.stubGlobal('window', {
      kunGui: {
        getSettings: vi.fn(async () => ({
          agents: { kun: { providerId: 'previous-provider', model: 'previous-model' } },
          codePromptPrefix: ''
        })),
        saveSettingsSilent,
        restartRuntime,
    ...
    expect(provider.sendUserMessage).toHaveBeenCalledWith(
      'thr_existing',
      expect.stringContaining('[Current user request]\nhello'),
      expect.objectContaining({ model: 'gpt-5.2', displayText: 'hello' })
    )
  })
  ...
  it('applies an override provider before sending from the write route', async () => {
    ...
    registryMock.getProvider.mockReturnValue(provider)
    const saveSettingsSilent = vi.fn(async () => ({
      agents: { kun: { providerId: 'custom-provider', model: 'custom-model' } },
      codePromptPrefix: ''
    }))
    const restartRuntime = vi.fn(async () => undefined)
    vi.stubGlobal('window', {
      kunGui: {
        getSettings: vi.fn(async () => ({
          agents: { kun: { providerId: 'bai', model: 'claude-sonnet-4-6' } },
          codePromptPrefix: ''
        })),
        saveSettingsSilent,
        restartRuntime,
    ...
    expect(provider.sendUserMessage).toHaveBeenCalledWith(
      'thr_existing',
      expect.stringContaining('[Current user request]\nmake a prototype'),
      expect.objectContaining({ model: 'custom-model', displayText: 'make a prototype' })
    )
  })
})

...
describe('thread usage formatting', () => {
  ...
  it('trusts explicit unavailable cache telemetry over zero counters', async () => {
    const runtimeRequest = vi.fn<RuntimeRequest>(async (path) => {
      if (path === threadUsagePath('thr_estimated')) {
        return {
          ok: true,
          status: 200,
          body: JSON.stringify({
    ...
    const usage = await loadThreadUsage('thr_estimated')

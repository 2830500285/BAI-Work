...
export type ThreadUsageSummary = {
  ...
  tokenEconomyTelemetryAvailable: boolean
  turns: number
}
...
function usageTelemetryFlag(value: unknown, fallback: boolean): boolean {
...
export async function loadThreadUsage(threadId: string): Promise<ThreadUsageSummary | null> {
  ...
  const hasBucketCacheTelemetry = usageTelemetryFlag(bucket.cache_telemetry_available, bucketCacheHitRate !== null)
  ...
  const cacheMissTokens = hasBucketCacheTelemetry
      ...
      : 0
  const cacheHitRate = hasBucketCacheTelemetry ? bucketCacheHitRate : null
  const lastTurnCacheHitRate = hasBucketCacheTelemetry ? usageRate(bucket.last_turn_cache_hit_rate) : null
  ...
  const tokenEconomyTelemetryAvailable = usageTelemetryFlag(
    bucket.token_economy_telemetry_available,
    hasFiniteNumber(bucket, 'token_economy_savings_tokens')
  )
  const tokenEconomySavingsTokens = tokenEconomyTelemetryAvailable
    ? usageNumber(bucket.token_economy_savings_tokens)
    : 0
  ...
  return {
    ...
    cacheTelemetryAvailable: hasBucketCacheTelemetry,
    totalTokens,
    costUsd,
    costCny,
    tokenEconomySavingsTokens,
    tokenEconomyTelemetryAvailable,
    turns
  }
}

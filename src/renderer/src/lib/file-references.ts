...
const FILE_REFERENCE_SCHEME = 'bai-work-file:'
const LEGACY_FILE_REFERENCE_SCHEME = 'mimo-work-file:'
...
export function parseFileReferenceHref(href: string | undefined): FileReferenceTarget | null {
  if (!href?.startsWith(FILE_REFERENCE_SCHEME) && !href?.startsWith(LEGACY_FILE_REFERENCE_SCHEME)) return null
  try {

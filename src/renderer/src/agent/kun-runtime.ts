...
function runtimeErrorForSseFailure(
  ...
  if (status !== undefined) {
    return {
      ...
      message: `BAI Work 运行时事件流连接失败（HTTP ${status}）。当前回复没有完成，请检查本地运行时或模型供应商后重试。`
    }
  }
  ...
  return {
    ...
    message: terminated
      ? 'BAI Work 运行时事件流已中断，当前回复没有完成。通常是模型供应商、自定义路由或本地运行时提前终止，请检查后重试。'
      : `BAI Work 运行时事件流出错，当前回复没有完成。请检查本地运行时或模型供应商后重试。详情：${detail}（stream ${streamId}）`
  }
}

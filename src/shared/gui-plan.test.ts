...
describe('gui-plan path validation', () => {
  it('accepts direct Markdown files under the plan directory', () => {
    ...
    expect(isGuiPlanCurrentRelativePath('.legacy-work/plan/login.md')).toBe(false)
  })
  ...
  it('produces a stable plan id from workspace and path', () => {
    expect(buildGuiPlanId('/tmp/ws', '.bai-work/plan/login.md')).toBe(
      '/tmp/ws:.bai-work/plan/login.md'
    )
    expect(buildGuiPlanId('/tmp/ws', '.bai-work/plan/Login.md')).toBe(
      buildGuiPlanId('/tmp/ws', '.bai-work/plan/login.md')
    )
  })

...
import {
  ...
  CODE_BAI_WORK_EXECUTION_GUARDRAILS
} from './app-settings-prompts'

describe('app settings prompts', () => {
  it('injects BAI Work execution guardrails for code prompts without a custom prefix', () => {
    ...
    expect(prompt).toContain(CODE_BAI_WORK_EXECUTION_GUARDRAILS)
  ...
  it('preserves the user configured code prompt prefix after the built-in guardrails', () => {
    ...
    expect(prompt.indexOf(CODE_BAI_WORK_EXECUTION_GUARDRAILS)).toBeLessThan(

...
const DEFAULT_GITHUB_OWNER = '2830500285'
const DEFAULT_GITHUB_REPO = 'BAI-Work'
...
let configuredChannel: GuiUpdateChannel = normalizeGuiUpdateChannel(
  envValue('BAI_WORK_UPDATE_CHANNEL') || undefined
)
...
function envUpdateUrl(channel: GuiUpdateChannel): string {
  const channelSpecific = envValue(`BAI_WORK_UPDATE_URL_${channel.toUpperCase()}`)
  const direct = channelSpecific || envValue('BAI_WORK_UPDATE_URL')
  return direct ? direct.replace(/\{channel\}/g, channel).replace(/\/?$/, '/') : ''
}

function githubOwner(): string {
  return envValue('BAI_WORK_GITHUB_OWNER') || DEFAULT_GITHUB_OWNER
}

function githubRepo(): string {
  return envValue('BAI_WORK_GITHUB_REPO') || DEFAULT_GITHUB_REPO
}

type UpdaterFeed =
  | { kind: 'generic'; key: string; url: string; options: { provider: 'generic'; url: string } }
  | {
      kind: 'github'
      key: string
      options: { provider: 'github'; owner: string; repo: string; channel: string }
    }

function updaterFeed(channel: GuiUpdateChannel): UpdaterFeed {
  ...
  if (direct) {
    return {
      ...
      key: `generic:${direct}`,
      url: direct,
      options: { provider: 'generic', url: direct }
    }
  }
  ...
  return {
    kind: 'github',
    key: `github:${owner}/${repo}:${channel}`,
    options: { provider: 'github', owner, repo, channel: 'latest' }
  }
}
...
function resolveGithubReleaseUrl(): string | null {
  const envRepo = normalizeGithubOwnerRepo(process.env.BAI_WORK_GITHUB_REPO?.trim() ?? '')
...
function macAutoUpdateAllowed(): boolean {
  ...
  if (process.env.BAI_WORK_ALLOW_UNSIGNED_UPDATES === '1') return true
...
async function runScheduledGuiUpdateCheck(): Promise<void> {
  ...
  backgroundCheckPromise = (async () => {
    try {
      ...
      console.warn('[bai-work updater] scheduled GUI update check failed:', error)
    } finally {
...
function configureUpdaterChannel(channel: GuiUpdateChannel, feed = updaterFeed(channel)): void {
  const normalized = normalizeGuiUpdateChannel(channel)
  const changed = normalized !== configuredChannel || feed.key !== configuredFeedUrl
  configuredChannel = normalized
  configuredFeedUrl = feed.key
...
function isGithubReleaseJson(value: unknown): value is GithubReleaseJson {
  return typeof value === 'object' && value !== null && !Array.isArray(value)
}

async function fetchGithubRelease(channel: GuiUpdateChannel): Promise<GithubReleaseJson> {
  const base = `https://api.github.com/repos/${encodeURIComponent(githubOwner())}/${encodeURIComponent(githubRepo())}`
  const url = channel === 'frontier' ? `${base}/releases?per_page=20` : `${base}/releases/latest`
  const res = await fetch(url, {
    headers: {
      Accept: 'application/vnd.github+json',
      'User-Agent': `bai-work/${app.getVersion()}`
    }
  })
  if (!res.ok) throw new Error(`GitHub release metadata returned ${res.status}.`)
  const body = await res.json() as unknown
  if (channel === 'frontier') {
    const release = Array.isArray(body)
      ? body.find((item) => isGithubReleaseJson(item) && item.draft !== true)
      : null
    if (isGithubReleaseJson(release)) return release
    throw new Error('GitHub has no published BAI Work releases.')
  }
  if (!isGithubReleaseJson(body)) throw new Error('GitHub returned invalid release metadata.')
  return body
}
...
async function checkManualUpdate(
  ...
  try {
    const feed = updaterFeed(channel)
    let latestVersion = ''
    let releaseDate = ''
    let releaseUrl = downloadPageUrl()
    if (feed.kind === 'generic') {
      const res = await fetch(`${feed.url}${platformManifestName()}`, {
        headers: {
          Accept: 'application/x-yaml,text/yaml,text/plain,*/*',
          'User-Agent': `bai-work/${currentVersion}`
        }
      })
      ...
      latestVersion = String(release.tag_name ?? '').trim().replace(/^v/i, '')
      releaseDate = String(release.published_at ?? '').trim()
      releaseUrl = String(release.html_url ?? '').trim() || releaseUrl
    }
    if (!latestVersion) {
      return {
        ok: false,
...
export function initializeGuiUpdater(
  ...
  autoUpdater.logger = {
    info: (message?: unknown) => console.info('[bai-work updater]', message),
    warn: (message?: unknown) => console.warn('[bai-work updater]', message),
    error: (message?: unknown) => console.error('[bai-work updater]', message)
  }
  ...
  nativeAutoUpdater?.on?.('before-quit-for-update', () => {
    void runBeforeInstallUpdate().catch((error) => {
      console.warn('[bai-work updater] failed to stop runtimes before update quit:', error)
    })
  })

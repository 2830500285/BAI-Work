...
describe('ClawRuntime', () => {
  ...
  it('handles Feishu model commands locally for the current IM channel', async () => {
    ...
    expect(runtimeRequest).not.toHaveBeenCalled()
    expect(current().claw.channels[0].model).toBe('gpt-5.2')
    expect(send).toHaveBeenCalledWith(
      'oc_chat_a',
      { markdown: 'Claw IM model switched to `gpt-5.2`.' },
      { replyTo: 'om_inbound', replyInThread: false }
    )
  })
  ...
  it('resolves the default IM auto model to the current Kun model before starting a turn', async () => {
    ...
    settings.claw.im.enabled = true
    settings.claw.im.responseTimeoutMs = 2_000
    settings.agents.kun.providerId = 'bai'
    settings.agents.kun.model = 'claude-sonnet-4-6'
    settings.provider.providers = [
      ...settings.provider.providers,
      buildModelProvider({
        id: 'bai',
    ...
    const runtimeRequest = vi.fn(async (requestSettings: AppSettingsV1, path, init) => {
      expect(requestSettings.agents.kun.providerId).toBe('bai')
      expect(requestSettings.agents.kun.model).toBe('claude-sonnet-4-6')
      if (path === '/v1/threads/thr_bai/turns' && init?.method === 'POST') {
        const body = JSON.parse(init?.body ?? '{}') as { model?: string }
        expect(body.model).toBe('claude-sonnet-4-6')
        return { ok: true, status: 202, body: JSON.stringify({ threadId: 'thr_bai', turnId: 'turn_bai' }) }
      }
      if (path === '/v1/threads/thr_bai' && init?.method === 'GET') {
        return {
          ok: true,
          status: 200,
          body: JSON.stringify({
            ...
            turns: [
              {
                id: 'turn_bai',
                status: 'completed',
                items: [{ kind: 'assistant_text', text: 'hello from bai' }]
              }
            ]
          })
        }
    ...
    expect(result).toMatchObject({ ok: true, text: 'hello from bai' })
  })
  ...
  it('sends the channel intro before handling the first Feishu message', async () => {
    ...
    expect(welcomeCall[1].markdown).toContain('BAI Work')
  ...
  it('prepends the intro to the first webhook reply when no push channel exists', async () => {
    ...
    expect(reply).toContain('BAI Work')
  ...
  it('sends the latest generated workspace file to Feishu when the user asks for it', async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'bai-work-feishu-file-'))
  ...
  it('sends generated image tool output to Feishu for image requests', async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'bai-work-feishu-image-'))
    const imageDir = join(workspaceRoot, '.bai-work-images')
                  ...
                  items: [
                    {
                      ...
                      output: {
                        files: [{
                          ...
                          relativePath: '.bai-work-images/img-20260611000100-abcd.png',
                          mimeType: 'image/png'
                        }],
  ...
  it('returns generated files in the WeChat webhook reply for image requests', async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'bai-work-weixin-image-'))
    const imageDir = join(workspaceRoot, '.bai-work-images')
                  ...
                  items: [
                    {
                      ...
                      output: {
                        files: [{
                          absolutePath: imagePath,
                          relativePath: '.bai-work-images/img-20260611000200-beef.png',
                          mimeType: 'image/png'
                        }],
                        endpoint: 'generations'
                      },
  ...
  it('returns current-turn generated music files in the WeChat webhook reply for follow-up prompts', async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'bai-work-weixin-music-'))
    const mediaDir = join(workspaceRoot, '.bai-work-media')
                  ...
                  items: [
                    {
                      ...
                      output: {
                        files: [{
                          ...
                          relativePath: '.bai-work-media/music-20260612054704-78a2.mp3',
                          mimeType: 'audio/mpeg'
                        }]
                      },
                      isError: false
  ...
  it('does not return files from previous turns when the current IM turn produces none', async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'bai-work-weixin-stale-files-'))
    const imageDir = join(workspaceRoot, '.bai-work-images')
                  ...
                  items: [
                    {
                      ...
                      output: {
                        files: [{
                          absolutePath: imagePath,
                          relativePath: '.bai-work-images/img-20260611000300-cafe.png',
                          mimeType: 'image/png'
                        }]
                      },
                      isError: false
  ...
  it('returns generated speech files in the WeChat webhook reply for voice requests', async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'bai-work-weixin-speech-'))
    const speechDir = join(workspaceRoot, '.bai-work-media')
                  ...
                  items: [
                    {
                      ...
                      output: {
                        files: [{
                          ...
                          relativePath: '.bai-work-media/speech-20260612000100-feed.mp3',
                          mimeType: 'audio/mpeg'
                        }]
                      },
                      isError: false
  ...
  it('returns current-turn generated video files in the WeChat webhook reply for follow-up prompts', async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'bai-work-weixin-video-'))
    const mediaDir = join(workspaceRoot, '.bai-work-media')
                  ...
                  items: [
                    {
                      ...
                      output: {
                        files: [{
                          ...
                          relativePath: '.bai-work-media/video-20260612061000-c0de.mp4',
                          mimeType: 'video/mp4'
                        }]
                      },
                      isError: false

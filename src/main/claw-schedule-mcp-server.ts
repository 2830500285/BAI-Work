...
export async function runClawScheduleMcpServerFromArgv(argv: string[]): Promise<boolean> {
  ...
  const server = new McpServer(
    { name: 'bai-work-schedule', version: '0.1.0' },
  ...
  const registerListTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. List scheduled tasks managed by the currently running BAI Work app.'
        : 'List scheduled tasks managed by the currently running BAI Work app.'
    }, async () => {
      try {
  ...
  const registerCreateTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. Create a scheduled task in BAI Work. Supports one-time (`at`), daily, or interval schedules.'
        : 'Create a scheduled task in BAI Work. Supports one-time (`at`), daily, or interval schedules. When creating from an existing conversation, pass that conversation\'s provider_id, model, and reasoning_effort so the scheduled task keeps the same execution settings.',
      inputSchema: {
        ...
        provider_id: z.string().optional().describe('Optional model provider id configured in BAI Work settings'),
        model: z.string().optional().describe('Optional model id, e.g. claude-sonnet-4-6 / gpt-5.2'),
  ...
  const registerUpdateTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. Update an existing BAI Work scheduled task.'
        : 'Update an existing BAI Work scheduled task.',
      inputSchema: {
  ...
  const registerDeleteTool = (name: string): void => {
    server.registerTool(name, {
      description: name.startsWith('claw_')
        ? 'Legacy alias. Delete a scheduled task from BAI Work.'
        : 'Delete a scheduled task from BAI Work.',
      inputSchema: {
        task_id: z.string().min(1).describe('Task id returned by gui_schedule_list or gui_schedule_create')
      }
    }, async ({ task_id }) => {

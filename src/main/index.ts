...
import mimoWorkLogoPng from '../asset/img/bai-work.png?url'
import mimoWorkMacLogoPng from '../asset/img/bai-work-mac.png?url'
import mimoWorkTrayPng from '../asset/img/bai-work-tray.png?url'
...
import { setBaiWorkUnexpectedExitHandler, type BaiWorkUnexpectedExitInfo } from './runtime/bai-work-adapter'
...
const startupTraceEnabled = process.env.BAI_WORK_STARTUP_TRACE === '1'
...
function resolveConfiguredApiKey(settings: AppSettingsV1): string {
  ...
  if (runtime.runtimeEngine === 'bai-work') {
    return runtime.mimo.apiKey.trim() || getActiveAgentApiKey(settings) || process.env.BAI_API_KEY?.trim() || ''
  }
  return getActiveAgentApiKey(settings) || process.env.BAI_API_KEY?.trim() || ''
}
...
function syncLoginItemSettings(settings: AppSettingsV1): void {
  ...
  try {
    ...
    console.warn('[bai-work] failed to update login item settings:', error)
...
async function showTurnCompleteNotification(
  ...
  const title = normalizeNotificationText(payload.title, 'BAI Work', 80)
...
function handleUnexpectedBaiWorkExit(info: BaiWorkUnexpectedExitInfo): void {
  void superviseBaiWorkCrash(info).catch((error: unknown) => {
    logError('bai-work-supervisor', 'supervised restart crashed', {
      message: error instanceof Error ? error.message : String(error)
    })
  })
}

async function superviseBaiWorkCrash(info: BaiWorkUnexpectedExitInfo): Promise<void> {
  ...
  publishRuntimeStatus({
    state: 'crashed',
    source: 'supervisor',
    message: `BAI Work runtime exited unexpectedly (${exitLabel}).`,
  ...
  try {
    ...
    if (!resolveConfiguredApiKey(settings) || !runtime.autoStart) {
      publishRuntimeStatus({
        state: 'stopped',
        source: 'supervisor',
        message: 'BAI Work runtime exited and automatic restart is unavailable (missing API key or auto-start disabled).'
      })
      return
    }
    let lastError = ''
    for (;;) {
      ...
      if (!verdict.allowed) {
        publishRuntimeStatus({
          ...
          source: 'supervisor',
          message: lastError
            ? `BAI Work runtime keeps crashing; automatic restarts are paused. Last error: ${lastError}`
            : 'BAI Work runtime keeps crashing; automatic restarts are paused. Check the runtime logs, then retry.',
          stderrTail: info.stderrTail
        })
        return
      }
      publishRuntimeStatus({
        ...
        maxAttempts: 3,
        message: `Restarting BAI Work runtime automatically (attempt ${verdict.attempt}/3).`
      })
      ...
      try {
        ...
        logWarn('bai-work-supervisor', `automatic restart attempt ${verdict.attempt} failed: ${lastError}`)
      }
    }
  } finally {
...
function startRuntimeWatchdog(): void {
  ...
  const timer = setInterval(() => {
    void runtimeWatchdogTick().catch((error: unknown) => {
      logWarn('bai-work-watchdog', 'watchdog tick failed', {
        message: error instanceof Error ? error.message : String(error)
      })
    })
...
async function runtimeWatchdogTick(): Promise<void> {
  ...
  try {
    ...
    publishRuntimeStatus({
      state: 'restarting',
      source: 'watchdog',
      message: 'BAI Work runtime stopped responding to health checks; restarting it.'
    })
    try {
      ...
      publishRuntimeStatus({
        state: 'failed',
        source: 'watchdog',
        message: `BAI Work runtime is unresponsive and the automatic restart failed: ${
...
function queueRuntimeSettingsApply(prev: AppSettingsV1, next: AppSettingsV1): void {
  ...
  const task = previousTask
    ...
    .catch((error: unknown) => {
      logWarn('settings-apply', 'Failed to apply BAI Work runtime settings in background', {
        message: error instanceof Error ? error.message : String(error)
      })
    })
    .finally(() => {
...
function queueRuntimeMcpConfigApply(settings: AppSettingsV1): void {
  ...
  const task = previousTask
    ...
    .catch((error: unknown) => {
      logWarn('mcp-config', 'Failed to apply BAI Work MCP config change in background', {
        message: error instanceof Error ? error.message : String(error)
      })
    })
    .finally(() => {
...
async function resolveManagedKunLaunchSettings(
  ...
  lastAppliedSettings = next
  logWarn(source, `BAI Work runtime port ${runtime.port} is unavailable; using ${resolved.port} for the managed runtime`, {
...
async function ensureKunRuntime(settings: AppSettingsV1): Promise<AppSettingsV1> {
  ...
  if (!hasApiKey) {
    throw runtimeJsonError(
      'missing_api_key',
      'BAI API key is required before BAI Work can start.'
    )
  }
  if (!runtime.autoStart) {
    throw runtimeJsonError(
      'runtime_offline',
      'BAI Work runtime is offline. Enable automatic startup in Settings.'
    )
  }
  ...
  try {
    ...
    console.error('[bai-work] failed to start runtime:', e)
    throw e
  }
  ...
  if (!started) {
    throw runtimeJsonError(
      'runtime_unhealthy',
      'BAI Work runtime did not become healthy after launch.'
    )
  }
...
async function restartRuntimeOnce(settings: AppSettingsV1): Promise<void> {
  ...
  try {
    ...
    console.error('[bai-work] failed to restart BAI Work runtime:', e)
    throw e
  }
  ...
  if (!healthy) {
    throw runtimeJsonError(
      'runtime_unhealthy',
      'BAI Work runtime did not become healthy after restart.'
    )
  }
...
function createWindow(options: { suppressInitialShow?: boolean } = {}): void {
  ...
  mainWindow.webContents.on('preload-error', (_event, preloadPath, error) => {
    const message = error instanceof Error ? error.message : String(error)
    console.error(`[bai-work] failed to load preload ${preloadPath}:`, error)
...
function validateRuntimeSettingsForApply(next: AppSettingsV1): string | null {
  ...
  if (!Number.isInteger(runtime.port) || runtime.port < 1 || runtime.port > 65_535) {
    return `BAI Work runtime port must be an integer between 1 and 65535 (got ${String(runtime.port)})`
  }
...
async function restartManagedRuntimeForSettingsChange(
  ...
  if (!resolveConfiguredApiKey(next) || !runtime.autoStart) {
    publishRuntimeStatus({
      ...
      message: 'BAI Work runtime was stopped: the new settings have no API key or auto-start is disabled.'
    })
    return
  }
  ...
  try {
    ...
    const healthy = await waitForKunHealth(launchSettings, 20_000)
    if (!healthy) {
      throw new Error('BAI Work runtime did not become healthy after the settings change')
    }
    noteRuntimeHealthy('settings-apply')
    publishRuntimeStatus({ state: 'running', source: 'settings-apply' })
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e)
    logWarn('settings-apply', `BAI Work runtime restart failed after settings change: ${message}`)
...
async function rollbackRuntimeSettingsAfterFailedApply(
  ...
  try {
    ...
    publishRuntimeStatus({
      ...
      source: 'settings-apply',
      rolledBack: true,
      message: `The new settings failed to apply (${failureMessage}); BAI Work is running on the previous settings again.`
    })
  } catch (error) {
    publishRuntimeStatus({
      state: 'failed',
...
async function restartManagedRuntimeForMcpConfigChange(settings: AppSettingsV1): Promise<void> {
  ...
  try {
    ...
    const healthy = await waitForKunHealth(launchSettings, 20_000)
    if (!healthy) {
      throw new Error('BAI Work runtime did not become healthy after the MCP config change')
    }
    noteRuntimeHealthy('mcp-config')
    publishRuntimeStatus({ state: 'running', source: 'mcp-config' })
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e)
    logWarn('mcp-config', `BAI Work runtime restart failed after MCP config change: ${message}`)
    publishRuntimeStatus({
      state: 'failed',
      source: 'mcp-config',
      message: `BAI Work runtime failed to restart after the MCP config change: ${message}. Check the MCP config file, then retry.`
    })
  }
}
...
async function waitForManagedRuntimeReadyBeforeStop(
  ...
  if (!healthy) {
    logWarn(source, 'BAI Work runtime did not become healthy before a managed restart; stopping it anyway')
    return
  }
  const idle = await waitForRuntimeTurnsIdle({ settings })
  if (idle === 'timeout') {
    logWarn(source, 'BAI Work runtime still has running turns after waiting; stopping it anyway')
  } else if (idle === 'unavailable') {
    logWarn(source, 'Could not verify BAI Work runtime turn idleness before a managed restart; stopping it anyway')
  }
}
...
app.whenReady().then(async () => {
  ...
  setBaiWorkUnexpectedExitHandler(handleUnexpectedBaiWorkExit)
  ...
  void loadGuiUpdaterModule().catch((error) => {
    console.warn('[bai-work updater] failed to initialize on startup:', error)
  })
  ...
  if (resolveConfiguredApiKey(initial)) {
    setTimeout(() => {
      void managedRuntimeAdapterForSettings(initial).resolveExecutable(initial).catch((err) => {
        console.warn('[bai-work] prewarm BAI Work runtime:', err)
      })
    }, 1500)
  }
  ...
  const message = error instanceof Error ? error.message : String(error)
  console.error('[bai-work] startup failed:', error)
  dialog.showErrorBox('BAI Work failed to start', message)
  app.quit()
})
}
...
app.on('window-all-closed', () => {
  void stopManagedRuntimes().catch((error) => {
    console.warn('[bai-work] failed to stop runtime:', error)
  })
  if (process.platform !== 'darwin') {
    app.quit()
  }

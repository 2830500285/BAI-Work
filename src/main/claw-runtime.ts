...
function resolveImModelAlias(value: string): string {
  ...
  if (normalized === 'pro') return 'claude-sonnet-4-6'
  if (normalized === 'flash') return 'gpt-5.2'
  return value.trim()
}
...
function imCommandHelpText(settings: AppSettingsV1): string {
  if (isChineseLocale(settings)) {
    return [
      ...
      '也支持 `-new`、`-help`、`-provider bai`、`-model claude-sonnet-4-6` 这种写法。'
    ].join('\n')
  }
  return [
    'Claw IM commands:',
    ...
    '`-new`, `-help`, `-provider bai`, and `-model claude-sonnet-4-6` are supported too.'
  ].join('\n')
}
...
export function imWelcomeText(settings: AppSettingsV1, channel?: ClawImChannelV1): string {
  ...
  const name = profile?.name.trim() || channel?.label.trim() || 'BAI Work'
  const description = profile?.description.trim() ?? ''
  if (isChineseLocale(settings)) {
    return [
      `你好，我是 ${name}，通过 BAI Work 连接到这个对话的 AI 助手。`,
  ...
  return [
    `Hi, I am ${name}, an AI assistant connected to this chat through BAI Work.`,
    ...(description ? [description] : []),
...
export class ClawRuntime {
  ...
  private async recentGeneratedFilesForThread(
    ...
    try {
      ...
      if (!detailRes.ok) {
        this.deps.logError('claw-feishu', 'Failed to read recent generated files from BAI Work thread', {
          ...context,
      ...
      this.deps.logError('claw-feishu', 'Failed to inspect BAI Work thread for recent generated files', {
        ...context,
        threadId: targetThreadId,
        message: errorMessage(error)
      })
  ...
  private async handleWebhook(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      if (url.pathname === '/claw/internal/gui-plan/create' && req.method === 'POST') {
        ...
        // native BAI Work `create_plan` tool. Reject legacy calls
        // loudly so older clients see a clear migration error.
        writeJson(res, 410, {
          ok: false,
          code: 'gui_plan_create_retired',
          message:
            'The /claw/internal/gui-plan/create endpoint is no longer active. Use the BAI Work create_plan tool.'
        })
        return
      }
      ...
      if (im.secret) {
        ...
        const rawHeaderSecret = req.headers['x-bai-work-secret'] ?? req.headers['x-mimo-work-secret']

...
import { installEccMappings } from '../services/ecc-mapping-service'
import { clearBaiRuntimeInstructionCache } from '../services/bai-runtime-instruction-service'
...
export function registerAppIpcHandlers(options: RegisterAppIpcHandlersOptions): void {
  ...
  ipcMain.handle(
    ...
    async (_, payload: unknown) => {
      ...
      try {
        ...
        clearBaiRuntimeInstructionCache()
  ...
  ipcMain.handle('skill:install-recommended', async (_, payload: unknown) => {
    ...
    const result = await installRecommendedSkillPackage(request.id)
    if (result.ok) clearBaiRuntimeInstructionCache()
    return result
  })

  ipcMain.handle('ecc:install', async (_, payload: unknown) => {
    const request = parseIpcPayload('ecc:install', eccInstallPayloadSchema, payload)
    const result = await installEccMappings(request)
    if (result.ok) clearBaiRuntimeInstructionCache()
    return result
  })

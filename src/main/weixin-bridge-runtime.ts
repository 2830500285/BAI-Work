...
function readWeixinPackageInfo(): WeixinPackageInfo {
  ...
  if (!packageJson) {
    throw new Error(
      'Built-in WeChat login component is missing. Reinstall BAI Work or rebuild with @tencent-weixin/openclaw-weixin bundled.'
    )
  }
...
function buildBaseInfo(): JsonRecord {
  ...
  return {
    ...
    bot_agent: `BAI-Work/${app.getVersion() || '0.0.0'}`
  }
}
...
async function postToMimoWorkWebhook(message: WeixinMessage, accountId: string): Promise<JsonRecord> {
  ...
  if (settings.webhookSecret) {
    ...
    headers['x-bai-work-secret'] = settings.webhookSecret
  }
  ...
  if (!res.ok || data.ok === false) {
    throw new Error(recordString(data, 'message') || `BAI Work webhook HTTP ${res.status}`)
  }
  return data
}
...
async function monitorWeixinAccount(accountId: string, signal: AbortSignal): Promise<void> {
  ...
  const dispatchToSender = (message: WeixinMessage, to: string, contextToken: string | undefined): void => {
    const task = async (): Promise<void> => {
      ...
      const result = await postToMimoWorkWebhook(message, account.accountId)

...
const launch: ClawScheduleMcpLaunchConfig = {
  appPath: '/Applications/BAI Work.app',
  execPath: '/Applications/BAI Work.app/Contents/MacOS/BAI Work',
  isPackaged: false
}

describe('claw schedule MCP config', () => {
  it('uses BAI Work config files by default', () => {
    expect(resolveKunConfigPath()).toBe(join(homedir(), '.bai-work', 'config.toml'))
    expect(resolveKunMcpJsonPath()).toBe(join(homedir(), '.bai-work', 'mcp.json'))
  })

  it('writes the gui_schedule server to the BAI Work MCP JSON config shape', () => {
  ...
  it('uses the macOS Electron helper for real app bundle paths', () => {
    expect(resolveClawScheduleMcpCommand(launch, 'darwin')).toBe(
      '/Applications/BAI Work.app/Contents/Frameworks/BAI Work Helper.app/Contents/MacOS/BAI Work Helper'
    )
    expect(resolveClawScheduleMcpCommand({
      appPath: '/tmp/bai-work-test-app',
  ...
  it('removes legacy config.toml claw_schedule blocks without touching other MCP servers', () => {
    const cleaned = removeLegacyClawScheduleTomlConfig(
      [
        ...
        'args = []',
        '',
        '# BAI Work plugin:mcp:claw-schedule START',
        '[mcp_servers.claw_schedule]',
        'command = "electron"',
        'args = []',
        '# BAI Work plugin:mcp:claw-schedule END',
        '',
        '[providers.bai]',
        'api_key = ""'
      ].join('\n')
    ...
    expect(cleaned).toContain('[providers.bai]')
    expect(cleaned).not.toContain('[mcp_servers.claw_schedule]')
    expect(cleaned).not.toContain('BAI Work plugin:mcp:claw-schedule')
  })

...
/**
 * Enabled, on-disk skill roots passed to the runtime. Builds the common
 * directory conventions (.agents/.claude/.codex/skills + global equivalents)
 * plus configured extra dirs and drops any the user toggled off. BAI Work does
 * not crawl another product's plugin cache on every turn; users can expose a
 * specific cache directory through extraDirs when they deliberately need it.
 * Precedence (earlier wins on duplicate skill id): project commons → global
 * commons → built-in skills → extra dirs.
 */
...
export async function listGuiSkills(
  ...
  try {
    ...
    const skillsByRoot = await Promise.all(roots.map(async (root) => {
      ...
      const loaded = await Promise.all(candidates.map(async (candidate) =>
        loadSkillSummary(candidate, root.scope).catch((error) => {
          validationErrors.push({ root: candidate, message: errorMessage(error) })
          return null
        })
      ))
      return loaded.filter((skill): skill is GuiSkillSummary => skill !== null)
    }))
    return {
      ok: true,
      skills: dedupeSkills(skillsByRoot.flat()),
      validationErrors
    }
  } catch (error) {
    return { ok: false, message: errorMessage(error) }
...
export function discoverMimoWorkBuiltinSkillRoots(): string[] {
  ...
  return uniqueStrings([
    process.env.BAI_WORK_BUILTIN_SKILL_DIR?.trim(),
    resourcesPath ? join(resourcesPath, 'BAI-Work-Skills') : '',

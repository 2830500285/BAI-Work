...
describe('write infographic service', () => {
  ...
  it('keeps BAI-compatible image prompts inside the local selection limit', async () => {
    const client = fakeClient()
    const result = await requestWriteInfographic(settingsWithImageGen({
      protocol: 'openai-images',
      model: 'bai-image-model'
    }), {
    ...
    expect(result.ok).toBe(true)
    expect(client.requests[0].prompt.length).toBeLessThan(7_000)

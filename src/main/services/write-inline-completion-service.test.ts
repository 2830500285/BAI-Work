...
describe('requestWriteInlineCompletion', () => {
  it('calls the BAI Chat Completions endpoint with cursor context', async () => {
    const fetchMock = vi.fn(async () =>
      new Response(JSON.stringify({ choices: [{ message: { content: ' only a test' } }] }), {
        status: 200,
    ...
    expect(body.messages[0].content).toContain('BAI Work inline writing')
  ...
  it('does not confuse lookalike hosts with the official BAI endpoint', async () => {
    const fetchMock = vi.fn(async () =>
    ...
    const result = await requestWriteInlineCompletion(
      createSettings({ baseUrl: 'https://api.b.ai.evil.test/beta' }),
      createRequest()
    )
    ...
    const [url, init] = fetchMock.mock.calls[0] as unknown as [string, RequestInit]
    expect(url).toBe('https://api.b.ai.evil.test/v1/chat/completions')
  ...
  it('records missing API key failures in the debug log', async () => {
    ...
    expect(debugEntries[0].prompt).toContain('BAI Work inline completion')
    expect(debugEntries[0].prompt.endsWith('# Draft\n\nThis is')).toBe(true)
  })

  it('preserves an explicit BAI completion model', async () => {
    const fetchMock = vi.fn(async () =>
  ...
  it('falls back to the selected provider baseUrl and BAI runtime model when write keeps defaults', async () => {
    const fetchMock = vi.fn(async () =>
    ...
    settings.agents.kun.model = 'gpt-5.2'
    settings.write.inlineCompletion.baseUrl = 'https://api.b.ai/v1'
    settings.write.inlineCompletion.model = 'claude-sonnet-4-6'
  ...
  it('uses an explicit model override when write disables model inheritance', async () => {
    const fetchMock = vi.fn(async () =>
  ...
  it('uses chat completions when the unified request may return an edit action', async () => {
    ...
    const request = {
      ...
      editCandidate: {
        ...
        endLine: 3,
        endColumn: 38,
        original: 'BAI Work keeps text editing local.'
      },
      recentEdits: [{
  ...
  it('does not send provider-specific thinking controls for custom chat completion models', async () => {
    const fetchMock = vi.fn(async () =>
      new Response(JSON.stringify({
        choices: [{
          message: {
  ...
  it('builds the unified action prompt without retrieval snippets when none are supplied', () => {
    ...
    expect(prompt).toContain('BAI Work inline completion')

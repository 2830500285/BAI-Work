...
const CLAW_SCHEDULE_MCP_MARKER_START = '# BAI Work plugin:mcp:claw-schedule START'
const CLAW_SCHEDULE_MCP_MARKER_END = '# BAI Work plugin:mcp:claw-schedule END'
...
export function resolveKunConfigPath(): string {
  return join(homedir(), '.bai-work', 'config.toml')
}

export function resolveKunMcpJsonPath(): string {
  return join(homedir(), '.bai-work', 'mcp.json')
}
...
async function readJsonFile(path: string): Promise<unknown | null> {
  ...
  try {
    ...
    throw new Error(`Failed to parse BAI Work MCP config at ${path}: ${message}`, { cause: error })
  }
}

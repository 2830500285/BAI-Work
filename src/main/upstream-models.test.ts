...
describe('upstream model picker list', () => {
  it('includes Kun config model profiles, aliases, and the configured agent model', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'bai-work-models-'))
    ...
    if (result.ok) {
      ...
      expect(result.modelGroups).toEqual(expect.arrayContaining([
        ...
        expect.objectContaining({
          providerId: 'bai',
          label: 'BAI',
          modelIds: expect.arrayContaining(['deepseek-v3.2'])
        })
      ]))
      const baiGroup = result.modelGroups?.find((group) => group.providerId === 'bai')
      expect(baiGroup?.modelIds).not.toContain('deepseek-chat')
      expect(baiGroup?.modelIds).not.toContain('deepseek-reasoner')
    }
  })

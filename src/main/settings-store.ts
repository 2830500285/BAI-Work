...
import { atomicWriteFile } from './services/atomic-write'
import {
...
const DEFAULT_WORKSPACE_ROOT = join(homedir(), 'BAI Work', 'default_workspace')
const DEFAULT_CLAW_CHANNELS_ROOT = join(homedir(), 'BAI Work', 'claw')
const DEFAULT_WRITE_WORKSPACE_ROOT_ABSOLUTE = expandHomePath(DEFAULT_WRITE_WORKSPACE_ROOT)
const SETTINGS_FILE_NAME = 'bai-work-settings.json'
  ...
  async load(): Promise<AppSettingsV1> {
    ...
    try {
      ...
      if (error instanceof SyntaxError) {
        ...
        if (backupPath) {
          console.warn(
            `[bai-work] Invalid settings JSON was replaced with defaults. Backup: ${backupPath}`
          )
        } else {
          console.warn(
            `[bai-work] Invalid settings JSON was replaced with defaults. Backup could not be written for ${sourcePath}.`
          )
        }
        return defaults
      }
